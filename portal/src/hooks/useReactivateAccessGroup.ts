...
import {
    ReactivateAccessGroupRequestError,
    reactivateAccessGroup,
} from "../api/reactivateAccessGroup"
import { useSession } from "../context/SessionContext"

export type ReactivateAccessGroupStatus =
    | { kind: "idle" }
    | { kind: "pending" }
    | { kind: "success" }
    | { kind: "denied" }
...
export interface UseReactivateAccessGroupResult {
    status: ReactivateAccessGroupStatus
...
const idleStatus: ReactivateAccessGroupStatus = { kind: "idle" }

// Mirrors useDeactivateAccessGroup's shape exactly (Phase 13E-B checkpoint
// 6's "Reactivate access group" control).
export function useReactivateAccessGroup(
    campaignId: string,
    onSuccess: () => void,
): UseReactivateAccessGroupResult {
        ...
        (accessGroupId: string) => {
            ...
            void reactivateAccessGroup(
                ...
                .catch((cause: unknown) => {
                    ...
                    if (cause instanceof ReactivateAccessGroupRequestError) {
                        if (cause.status === 401) {
                            setSnapshot({
                                campaignId: requestCampaignId,
                                status: idleStatus,
