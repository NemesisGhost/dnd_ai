...
This is an architecture restart. Phases 1 through 4 are complete, including Phase 4's initial ruleset/shared-character schema, its first corrections pass, and all three closeout passes (see [docs/PHASE4_VERIFICATION.md](docs/PHASE4_VERIFICATION.md); [docs/PHASE4_REMAINING_ISSUES.md](docs/PHASE4_REMAINING_ISSUES.md) is a closed historical record). Phase 5's location/dungeon schema, minimal knowledge-domain slice pulled forward from Phase 7, production invariants, resumed-waiter tests, and independent final-state assertions are complete. Formal test-infrastructure/tooling closeout has gone through thirteen review/pass cycles: a tenth review found that no Python thread can deliver the literal no-survivor guarantee the test-only `_BackgroundStatement` helper needs, a tenth pass replaced the worker thread with an independently terminable process, an eleventh review found that process-based redesign could still report false success and silently discard cleanup failures, an eleventh pass fixed it, a twelfth review found the eleventh pass's own verification-tooling claim didn't hold up to its exit code and its IPC redesign still relied on an abandonable thread, a twelfth pass fixed those findings, and a thirteenth review then found the twelfth pass's own missing-outcome classification, backend verification, and `Process.start()` failure handling were themselves still incomplete — a thirteenth pass fixed those findings and is verified locally, pending its own final-head CI confirmation (see [docs/PHASE5_VERIFICATION.md](docs/PHASE5_VERIFICATION.md); [docs/PHASE5_REMAINING_ISSUES.md](docs/PHASE5_REMAINING_ISSUES.md)). The Phase 6 repository-context modularization gate is closed; the Phase 5 formal-correctness gate is blocked pending that CI confirmation, so Phase 6 feature/schema work should not begin yet. See [README.md § Current Status](README.md#current-status).
