...
This is an architecture restart. Phases 1 through 5 are complete, including Phase 4's initial ruleset/shared-character schema, its first corrections pass, and all three closeout passes (see [docs/PHASE4_VERIFICATION.md](docs/PHASE4_VERIFICATION.md); [docs/PHASE4_REMAINING_ISSUES.md](docs/PHASE4_REMAINING_ISSUES.md) is a closed historical record), and Phase 5's location/dungeon schema, minimal knowledge-domain slice pulled forward from Phase 7, and six corrections passes — two pre-merge, four post-merge (see [docs/PHASE5_VERIFICATION.md](docs/PHASE5_VERIFICATION.md); [docs/PHASE5_REMAINING_ISSUES.md](docs/PHASE5_REMAINING_ISSUES.md) is now a closed historical record). Both Phase 6 entry gates (repository context modularization and Phase 5 correctness, including the concurrency-test cleanup helper's guaranteed teardown) are closed. See [README.md § Current Status](README.md#current-status).
