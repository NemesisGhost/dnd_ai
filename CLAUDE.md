...
This is an architecture restart. Phases 1 through 4 are complete (see [docs/PHASE4_VERIFICATION.md](docs/PHASE4_VERIFICATION.md); [docs/PHASE4_REMAINING_ISSUES.md](docs/PHASE4_REMAINING_ISSUES.md) is a closed historical record). Phase 5's gameplay features and production database invariants are implemented and CI-verified, but its formal closeout remains open for the concurrency-test cleanup guarantee in [docs/PHASE5_REMAINING_ISSUES.md](docs/PHASE5_REMAINING_ISSUES.md). The Phase 6 repository-context modularization gate is closed; Phase 6 feature/schema work must wait for that remaining Phase 5 test-infrastructure fix and a passing final workflow. See [docs/PHASE5_VERIFICATION.md](docs/PHASE5_VERIFICATION.md) and [README.md § Current Status](README.md#current-status).
