...
This is an architecture restart. Phases 1 through 6 are complete; Phase 7 (quests and knowledge) is current. Phase 5's location/dungeon schema, minimal knowledge slice, production invariants, resumed-waiter tests, and independent final-state assertions are complete and CI-verified; its long test-helper hardening history is preserved in [docs/PHASE5_VERIFICATION.md](docs/PHASE5_VERIFICATION.md). Phase 6 delivered events, interactions, checks, effects, event causality, and branch-aware inherited history across five increments, plus the first application-layer command handlers (`src/dnd_ai/commands`: `RecordEvent`, `PerformInteraction`, `ResolveCheck`) — the first code in this repository to mutate state through the application layer rather than hand-written SQL. Verified against AWS `dev`, 1,339 tests passing; see [docs/PHASE6_VERIFICATION.md](docs/PHASE6_VERIFICATION.md). Theoretical failures inside test-only cleanup primitives no longer block feature phases — apply the proportional verification policy in [docs/PLAN.md §23.1](docs/PLAN.md#231-phase-exit-review) and [§25.6](docs/PLAN.md#256-proportional-test-infrastructure-policy). See [README.md § Current Status](README.md#current-status).
