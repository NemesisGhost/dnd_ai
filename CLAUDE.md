...
This is an architecture restart. Phases 1 through 4 are complete, including Phase 4's initial ruleset/shared-character schema, its first corrections pass, and all three closeout passes (see [docs/PHASE4_VERIFICATION.md](docs/PHASE4_VERIFICATION.md); [docs/PHASE4_REMAINING_ISSUES.md](docs/PHASE4_REMAINING_ISSUES.md) is a closed historical record). Phase 5's location/dungeon schema, minimal knowledge-domain slice pulled forward from Phase 7, production invariants, resumed-waiter tests, and independent final-state assertions are complete, but **formal verification remains open**: a tenth review found that no Python thread can deliver the literal no-survivor guarantee the test-only `_BackgroundStatement` helper needs, a tenth pass replaced the worker thread with an independently terminable process, and an eleventh review then found that process-based redesign could still report false success and silently discard cleanup failures — a rewrite fixing the worker-outcome protocol and controller-side cleanup is pending its own CI confirmation (see [docs/PHASE5_VERIFICATION.md](docs/PHASE5_VERIFICATION.md) and [docs/PHASE5_REMAINING_ISSUES.md](docs/PHASE5_REMAINING_ISSUES.md)). The Phase 6 repository-context modularization gate is closed; the Phase 5 formal-correctness gate remains open, so do not begin Phase 6 feature/schema work. See [README.md § Current Status](README.md#current-status).
