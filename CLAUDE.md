...
This is an architecture restart. Phases 1 through 5 are complete; Phase 6 (events and interactions) is current. Phase 5's location/dungeon schema, minimal knowledge slice, production invariants, resumed-waiter tests, and independent final-state assertions are complete and CI-verified; its long test-helper hardening history is preserved in [docs/PHASE5_VERIFICATION.md](docs/PHASE5_VERIFICATION.md). Phase 6's five original increments (events, interactions, checks, effects, event causality, branch-aware inherited history, and the first application-layer command handlers in `src/dnd_ai/commands`) went through a second, more critical exit review that found production defects — missing recorded-event immutability, a concurrency race in `resolve_check()`, incomplete interaction-resolution wiring, and two stale reverse-guard/test-coverage gaps. A correction pass (revisions 065–068) closed all of them and was opened as PR #16. A further review of that PR found three more production lifecycle gaps — a recorded event could still be deleted (directly or via `core.entities` cascade), a completed interaction's status could revert to `initiated` and reopen its append-only structural records, and `resolve_check()` did not correctly lock or validate the parent interaction before recording a check result. A second correction pass (revisions 069–070) closed all three. A further review then found a multi-check interaction stayed `initiated`, not `resolving`, between its first and last check result, leaving the first check's own append-only records still editable; a third correction pass (revision 071) closed it, so `interaction.interactions.status` now actually moves `initiated → resolving → resolved` — but only when driven through `resolve_check()`. A fourth review found the database itself never enforced that transition (any other caller inserting a check result could leave the interaction stuck at `initiated`) and never stopped a resolved interaction from acquiring a brand new, unanswerable check request; a fourth correction pass (revision 072) closed both, moving the status transition into the database as an `AFTER INSERT` trigger and adding matching `BEFORE INSERT` guards on `actions`/`targets`/`check_requests`. See [docs/PHASE6_VERIFICATION.md](docs/PHASE6_VERIFICATION.md) for the full account, current test count, and outstanding CI-verification status. **Do not describe Phase 6 as complete, merged, or CI-confirmed until that document's "Verification status" section says so.** Theoretical failures inside test-only cleanup primitives no longer block feature phases — apply the proportional verification policy in [docs/PLAN.md §23.1](docs/PLAN.md#231-phase-exit-review) and [§25.6](docs/PLAN.md#256-proportional-test-infrastructure-policy). See [README.md § Current Status](README.md#current-status).
