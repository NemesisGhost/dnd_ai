...
This is an architecture restart. Phases 1 through 4 are complete, including Phase 4's initial ruleset/shared-character schema, its first corrections pass, and all three closeout passes (see [docs/PHASE4_VERIFICATION.md](docs/PHASE4_VERIFICATION.md); [docs/PHASE4_REMAINING_ISSUES.md](docs/PHASE4_REMAINING_ISSUES.md) is a closed historical record). Phase 5's location/dungeon schema, minimal knowledge-domain slice pulled forward from Phase 7, and three corrections passes are implemented, merged, and CI-verified, but a fourth review found one unresolved dungeon-area concurrency race; see [docs/PHASE5_VERIFICATION.md](docs/PHASE5_VERIFICATION.md) and the reopened [docs/PHASE5_REMAINING_ISSUES.md](docs/PHASE5_REMAINING_ISSUES.md). Do not begin [Phase 6: events and interactions](docs/PLAN.md#phase-6-events-and-interactions) feature/schema work until the Phase 5 closeout and Phase 6 context-modularization entry gates both close. See [README.md § Current Status](README.md#current-status).
