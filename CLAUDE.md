...
- **Compute:** one shared container image (`Dockerfile`) is built for the API, background worker, Discord adapter, and one-off jobs including migrations, with different entrypoints selecting the role — see [docs/DEVELOPMENT.md §2](docs/DEVELOPMENT.md#2-repository-layout). `src/dnd_ai/api` has no committed source yet, so migrations are the only role that exists today.
