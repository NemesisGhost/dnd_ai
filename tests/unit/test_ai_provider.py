...
OpenAiCompatibleProvider` — the one real AI provider Phase 12 ships
(`FakeAiProvider` is exercised everywhere else; this file is the only place
that constructs the real class). Never contacts a real endpoint: every test
monkeypatches `httpx.post` itself, so no network call and no real API key is
required. See that module's own docstring for why one class serves both
hosted OpenAI and a locally hosted, OpenAI-API-compatible model server.
"""
...
from dnd_ai.domain.context_assembly import NpcConversationContext
