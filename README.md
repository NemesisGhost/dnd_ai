...
The architecture is established and the database foundation is being implemented. Phases 1 (database bootstrap), 2 (core world platform), 3 (timelines and campaigns), and 4 (rules and shared characters, including its corrections pass and all three closeout passes — see [docs/PHASE4_VERIFICATION.md](docs/PHASE4_VERIFICATION.md)) are complete, verified against the deployed AWS `dev` environment including a successful GitHub Actions run. The current target is [Phase 5: locations and dungeon play](docs/PLAN.md#phase-5-locations-and-dungeon-play).
