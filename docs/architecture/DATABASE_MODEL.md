...
- `security.access_groups` — campaign-scoped named sets such as livestream observers, former players or a GM-curated lore audience. `name` is 1-200 characters (`ck_access_groups_name_length`), unique per campaign (case-sensitive); an optional `description` is 1-2000 characters when present (`ck_access_groups_description_length`, added by revision 106/Phase 13E-B checkpoint-6 correction) — a blank value normalizes to `NULL` before storage rather than an empty string. Revision 106 deploys this bound against a schema (080-105) that never enforced one: it backfills any pre-existing empty-string description to `NULL` (the same normalization already applied, harmlessly, retroactively) and blocks outright — a deliberate `RuntimeError` naming the affected `access_group_id`(s), never a silent truncation — if a pre-existing description already exceeds 2000 characters; the constraint is added `NOT VALID` then `VALIDATE CONSTRAINT`ed separately so validating existing rows never blocks concurrent reads/writes. Carries `lifecycle_status_id` (`core.lifecycle_statuses`, added by revision 105/Phase 13E-B checkpoint 6) so a group can be deactivated (`archived`) without being physically deleted, and later reactivated (`active`) — the same archive/restore pattern every other long-lived entity in this schema uses, applied here for the first time to a security concept. Reactivation flips only this column: it never reopens a membership or un-revokes a grant deactivation already closed.
