...
`narrative.event_causes` links an event to a prior event (`cause_event_id`) or, until `interaction.interactions` exists, a free-text `cause_description` placeholder — the same pattern Phase 5 used for `knowledge.entity_knowledge.learned_source`; a real `cause_interaction_id` reference is a later Phase 6 increment's job, not built yet.
...
That is the target model, not a requirement every typed-state table must already meet. Phase 4's `campaign.character_state`, `.character_conditions`, and `.character_resources` are current-state snapshots with no event-linked history columns at all: they predate `narrative.events` and each table instead enforces "one row per (timeline, target)" directly through its primary key — a gap Phase 6 has not yet closed for these three (see below). This is correct for their phase, not a gap to silently work around — do not add `effective_from_event_id`/`effective_to_event_id` to a table merely because this section names them as the general shape; rule 6 (state changes need a causal event, committing atomically) is a transaction-boundary guarantee the command layer provides, not a column these tables were missing.

Phase 6 revision 060 extended the five Phase-5 dungeon-domain state tables below with `last_event_id UUID FK NULL REFERENCES narrative.events(event_id) ON DELETE SET NULL` — the provenance reference this section calls for, not full interval history. `campaign.character_state`/`.character_conditions`/`.character_resources` do not have it yet; closing that is unstarted Phase 6 work, tracked the same way this document already tracks other open items.
...
## 27. Reconciliation notes (Phase 6, first increment)

Phase 6 ("Events and interactions") is being delivered as a sequence of independently reviewable increments rather than one change, the same way Phase 5 was. This section covers only the first: `narrative.events` and its five satellite tables (revision 057), `campaign.timelines.branch_event_id` (revision 058), `campaign.effective_events()` (revision 059), and `last_event_id` provenance on the five Phase-5 dungeon-domain state tables (revision 060). Interactions/checks/consequences, replacing the Phase 5 `learned_source`/`discovery_method` free-text placeholders, conditional-route evaluation, and the first command/service layer are later increments, not covered here.

**No command/service layer exists yet.** `src/dnd_ai` contains persistence (SQLAlchemy Core table metadata, migrations, seeds) and config only — no `commands`, `services`, or API app, confirmed before this increment was built. `SYSTEM_ARCHITECTURE.md` §5–7's command/transaction-boundary design is therefore still a target, not yet implemented anywhere; this increment proves the *schema* supports atomic event + typed-state commits (a hand-written multi-statement transaction, forced to fail partway through, leaves no partial write — `tests/database/test_event_state_atomicity.py`) without yet building the command layer that will make that pattern reusable application code.
...
**Not built in this increment, and why:**
- `narrative.events.corrected_by_event_id` / any correction-linkage column — `docs/ENTITY_LIFECYCLE.md` §15's `corrected` status exists as a value, but no Phase 6 exit criterion requires the linkage mechanism yet.
- `narrative.event_causes.cause_interaction_id` — `interaction.interactions` does not exist yet (a later increment); `cause_description` is a free-text placeholder in the meantime, the same pattern Phase 5 used for `knowledge.entity_knowledge.learned_source`.
- `last_event_id` on `campaign.character_state`/`.character_conditions`/`.character_resources` — PLAN.md's Phase 6 first-time obligations name only the five dungeon-domain state tables explicitly; the character-state tables are an equally valid target for the same provenance column but were not named, and adding it was left to whichever later increment actually produces character-affecting events.
- Replacing `knowledge.entity_knowledge.learned_source` / `knowledge.party_discoveries.discovery_method` — the docs are explicit these wait until *both* `interaction.interactions` and `narrative.events` exist; only the latter does yet.
