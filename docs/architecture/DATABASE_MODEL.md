...
- **Discovery source/provenance is a free-text placeholder, not real provenance.** `knowledge.entity_knowledge.learned_source` and `knowledge.party_discoveries.discovery_method` are `TEXT` columns holding a human-readable guess ("search_check", "told_by_npc") until `interaction.interactions`/`narrative.events` exist to reference properly (Phase 6). Revision 045 validates that the *timestamp* columns next to them (`learned_at_world_time_id`, `discovered_at_world_time_id`) belong to the right world, but does nothing to make the source/method columns machine-checkable — that is still Phase 6/7 work.
...
| `047_realm_conditional_routes` | Two completeness gaps: the `realm` location kind DOMAIN_MODEL.md §9.1 lists (revision 038 registered the other six no-subtype-table kinds but omitted this one by oversight), and the descriptive half of conditional routes PLAN.md §9.2 names (`is_conditional`/`condition_description` on `world.area_connections` — evaluating the condition is deferred to Phase 6/7, recorded explicitly in PLAN.md rather than left silent). |

**Judgment call carried forward, not fully resolved:** the knowledge-domain Phase 5/Phase 7 boundary (temporal validity of knowledge items; real discovery source/provenance) is now documented explicitly in §15 rather than left implicit, but neither gap was closed — both remain genuinely Phase 7's job.
