...
Every rule-content row's `ruleset_version_id` (or, for `rules.subclasses`/`rules.features`, the class/subclass/species association it is scoped by) must be immutable once set, using the same `core.enforce_immutable_columns()` trigger revision 030 introduced for world/timeline/party/campaign scope — a cross-version-consistency trigger only re-validates a *child* row at that child's own insert/update, so the *parent* row's identity must not change out from under already-valid children instead of needing a transactional revalidate-and-rebuild path. Revision 033 implements this policy for most definitions, but currently omits `rules.creature_types`, `rules.languages`, and `rules.feats`; that implementation gap is an active Phase 4 blocker. `character.character_builds.character_id`/`ruleset_version_id` and `character.character_spellcasting_profiles.character_build_id` are immutable for the same reason. Associations that are not identity (e.g. `rules.classes.primary_ability_id`, `rules.skills.ability_id`, `rules.spells.damage_type_id`) remain mutable — their owning row's trigger re-checks the relationship when that row changes.
