...
## 27. Reconciliation notes (Phase 6, increments 1–2)
...
Replacing the Phase 5 `learned_source`/`discovery_method` free-text placeholders, conditional-route evaluation, and the first command/service layer are later increments, not covered here.
...
**Replacing `knowledge.entity_knowledge.learned_source` / `knowledge.party_discoveries.discovery_method`** still hasn't happened — both `interaction.interactions` and `narrative.events` now exist, so this is unblocked, but is deliberately its own increment (increment 3) rather than folded into increment 2.
