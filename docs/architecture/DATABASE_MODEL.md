...
**Not yet built by Phase 12:** the audience-aware synthesis service (GM briefs, player-character question answers, observer-safe summaries) PLAN.md's Phase 12 "Deliver" list also names, and the corresponding exit criterion ("the same question produces appropriately different GM, player-character, and observer answers"). This phase delivered the reference corpus in full and one narrow NPC-conversation use case (deterministic, audience-filtered context assembly; one AI provider; structured output; the proposal/approval pipeline with a working auto-approve/requires-approval split) — the synthesis layer over `dnd_ai.queries.summary`/`.quest`/`.character` remains for a follow-up pass.
