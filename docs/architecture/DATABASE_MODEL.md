...
- `interaction.interactions` — not entity-rooted, unlike `narrative.events`; a high-volume log record (same reasoning DATABASE_MODEL.md §9 gives for `world.area_connections`/etc. not being entities). Scoped to `timeline_id`/`campaign_id`/`session_id` like events; `resulting_event_id` (nullable) is the event this interaction produced, when its outcome was significant enough to promote (§12's event-granularity guidance) — most interactions have none. `status` (`initiated`/`resolving`/`resolved`/`failed`/`cancelled`) is irreversible once it reaches one of the three terminal values (revision 070, see §27) — otherwise a status revert could reopen the append-only structural records below.
...
`src/dnd_ai/commands/interactions.py`'s `resolve_check()` implements this per check_request: it locks the parent interaction before recording a result (rejecting a check resolved against an interaction that has already reached a terminal status), links any event it produces to the interaction (`resulting_event_id`, a `state_change` consequence), and only moves the interaction itself to `resolved` once every check_request across all of its actions has a result — never before, and (status irreversibility, above) never more than once.
...
## 27. Reconciliation notes (Phase 6, increments 1–5, plus two exit-review correction passes)
