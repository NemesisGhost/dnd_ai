...
#### Foundry hybrid pairing (migration 100, Phase 11R workstream D)

docs/PLAN.md §23.5's replacement for the single shared `FoundrySystem` credential above: individually paired devices and short-lived access tokens, in four new `security` schema tables (not `integration`, to sit alongside the other credential tables workstream A/B added). Schema and commands only as of this workstream — `system_key_hash` is untouched, and nothing here is yet wired into `dnd_ai.api.auth.get_authenticated_user_id` (Phase 11R workstream F wires the `foundry_device`/`foundry_access` principal types and retires the legacy credential per docs/PLAN.md Workstream 11R item 7).
