...
`verify` performs no PostgreSQL state mutation by default — every one of
its checks, including the Alembic-revision check, is non-mutating. Its one
destructive exception, an actual `alembic upgrade head` run, only executes
when `--confirm-migrate` is explicitly passed (see "Verification" below).
...
`verify` runs the complete acceptance battery in one command — nothing here is left as "adapt the query yourself". **It performs no PostgreSQL state mutation by default**: every check below, including the Alembic-revision check, only reads. The one exception — actually running `alembic upgrade head` — is opt-in and DESTRUCTIVE (see below).
...
- **The current Alembic revision** — `alembic current`, which only reads `core.alembic_version`. This always runs and performs no PostgreSQL state mutation.
- **An actual `alembic upgrade head` run is available but opt-in and DESTRUCTIVE** — pass `--confirm-migrate` to run it. It proves migration machinery works end-to-end (connecting, `SET ROLE migration_owner`, reading/writing `core.alembic_version`). It does **not**, by itself, prove `CREATE ON DATABASE` works: when the dump is already at head, nothing between the restored revision and head needs to create an extension. The privilege check above already proved that directly; nothing here adds a throwaway `CREATE EXTENSION` purely to re-prove it. **Without `--confirm-migrate`, `verify` never runs migrations, and its entire acceptance battery is non-mutating.** `--confirm-migrate` is checked immediately before that one subprocess, consistent with every other destructive flag in this tool. (`--confirm-migrate` is not preflight — it runs the real migration command, so it is not gated behind the "image must already exist" guard the way the preflight-only migration-target check is; a build here, if the image happens to be missing, is expected, not a surprise.)
...
| `verify` run without `--confirm-migrate` | reflects only the non-mutating checks | No migration ran; the migration-machinery step is printed as skipped, not reported as passed |
| `verify --confirm-migrate`'s `alembic upgrade head` fails | nonzero | A real migration attempt ran against this database and failed; every other check's result is still reported |
