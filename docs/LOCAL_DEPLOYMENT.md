...
**Upgrading an existing installation through migration `105_world_ownership_scope`** (ADR 0014): every pre-existing world is backfilled onto one legacy ownership scope with **zero memberships** — the migration cannot safely guess which human should own it. After the migration runs, an operator must claim that scope by running `uv run python scripts/claim_legacy_ownership_scope.py --user-id <the platform administrator's user_id>` (DB-direct, same posture as `bootstrap_admin.py`) before any ownership-scope-scoped authorization check should be treated as meaningful for pre-existing worlds. This is a one-time step per installation; it refuses to run again once the scope already has an active owner.
