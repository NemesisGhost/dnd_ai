...
Checkpoint update: 13E-B's invitation-management slice is now delivered alongside the previously completed role, membership, relationship, direct-grant, access-group, and audit-history work. Campaign managers can list outstanding invitations, issue one-time bearer tokens, revoke outstanding invitations, and use the portal Access page to manage them. Authenticated human users can accept a token through `/campaign-invitations/accept`; the token remains request-body only, is shown exactly once on issuance, is stored only as a hash server-side, is never placed in browser storage or a URL, and acceptance grants membership only — never a role.
