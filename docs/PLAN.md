...
**Status: Partially implemented.** Delivered: the full rules/reference corpus (registration and rights metadata, immutable source/hash retention, structured-extraction ingestion, chapter/section/page citation, PostgreSQL full-text retrieval, campaign/edition/house-rule filtering, removal, and retrieval auditing — `core.source_documents`, `ai.reference_passages`, `.reference_source_campaigns`, `.reference_retrievals`/`.reference_retrieval_results`, migration `094_reference_corpus`); the AI agent/context/proposal schema (`ai.agents`, `.agent_roles`, `.agent_assignments`, `.prompt_templates`, `.prompt_fragments`, `.context_requests`, `.context_snapshots`, `.generated_outputs`, `.proposed_changes`, `.change_reviews`, migration `093_ai_domain`); one NPC-portrayal/conversation use case (`dnd_ai.domain.context_assembly`, `dnd_ai.domain.ai_provider` — `FakeAiProvider` for tests, `AnthropicAiProvider` for the one real provider — `dnd_ai.commands.ai_npc`); the proposed-change validation/approval pipeline (`dnd_ai.commands.ai_proposals`, `dnd_ai.commands.knowledge.reveal_knowledge_to_party` as the one wired target command, auto-approve vs. requires-approval risk classification by knowledge sensitivity); and API routes for all of the above (`dnd_ai.api.reference_corpus`, `dnd_ai.api.ai_npc`). Remaining: the audience-aware synthesis service (GM briefs, player-character question answers, observer-safe summaries) and its exit criterion (same question, different GM/player/observer answers); quest-state in NPC-conversation context (encounter and relationship state are already included; quest state is not); a second proposal kind beyond `reveal_knowledge`; and, per [§24.0](#240-verification-policy), a deliberate real-provider smoke-verification run against the live Anthropic API and a recorded `docs/PHASE12_VERIFICATION.md` — not done as of this pass, which was verified against the full local suite (migrations, `tests/unit`, `tests/database`) only, with `FakeAiProvider` standing in for every automated test's provider call.
