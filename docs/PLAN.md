...
| 11 | Partially implemented (all 6 workstreams delivered, not yet CI-closed) | Foundry-user identity linking (`link_foundry_identity`), adapter authentication (`issue_foundry_system_key`, `FoundrySystem` credential scheme — every existing command/query endpoint is Foundry-reachable), a campaign-scoped combat-sync endpoint (`apply_foundry_combat_sync_endpoint`), sync-state retrieval (`sync_state_endpoint`), current-location/encounter retrieval (`CharacterView.active_encounter_id`), and non-combat HP/condition/resource commands (`dnd_ai.commands.character_state`) | CI verification and `docs/PHASE11_VERIFICATION.md` |
...
**Status: Partially implemented — all six workstreams delivered and locally verified; not yet closed per [§24.0](#240-verification-policy) (no CI run on this work's final head commit, no `docs/PHASE11_VERIFICATION.md` yet).** Implemented: integration persistence contracts and API/domain command services (Phase 9-10); Foundry-user-to-platform-user identity linking (workstream 1); a real Foundry-adapter authentication scheme (workstream 2); a campaign-scoped combat-sync endpoint (workstream 3); sync-state retrieval (workstream 4); current location/encounter retrieval (workstream 5); non-combat HP/condition/resource commands (workstream 6, below). Remaining: the CI/verification-record close-out itself.
...
Every workstream above is delivered, and the phase's original deliverables list is fully covered (the "associate Foundry ... records"/"submit interactions ... through the API" bullets were resolved as already-substantially-covered once workstream 2 made every existing command/query endpoint Foundry-reachable, rather than needing dedicated new work). Per [§24.0](#240-verification-policy)/[§24.1](#241-phase-exit-review), that is necessary but not sufficient to close the phase: it still needs a CI run on the final head commit and a recorded `docs/PHASE11_VERIFICATION.md` before the status line above may say "Complete" — not yet done as of this workstream, since this work has not been pushed for CI to verify.
