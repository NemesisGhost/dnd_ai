...
- synchronize the minimum required character HP, conditions, resource use outside a combat turn (inventory already covered by Phase 9's item commands; combat-turn HP/condition/resulting-condition already covered by workstream 3; healing, non-combat HP adjustment, and resource-use tracking have no command yet)
