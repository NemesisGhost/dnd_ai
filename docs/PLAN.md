...
**Progress.** Workstream 1 (`080_security_identity_and_access`) delivered the `security.*` schema. Workstream 2 (`src/dnd_ai/domain/access.py`) delivered the effective-access resolver, including the rule that a campaign resolves access only against its own pinned timeline — never an unrelated timeline, branch, or a different world's — documented in that module and in [DATABASE_MODEL.md §19.7](architecture/DATABASE_MODEL.md#197-effective-access-resolution). Workstream 3 (`src/dnd_ai/api/`) delivered the FastAPI application skeleton: `/healthz` (process liveness) and `/readyz` (a real database round trip through the normal engine wiring, failing closed with a fixed, non-secret body — safe for a Compose health check), a non-disclosing error contract (request-validation failures echo only a field's location and pydantic's error-type code, never the rejected value), correlation IDs, and one-transaction-per-request dependency. Application configuration (`src/dnd_ai/config.py`) is namespaced under `DND_AI_*`, rejects unrecognized application variables, and refuses to start with the local development database once `DND_AI_ENVIRONMENT=production`. Still to come are authentication verification, command/query endpoints and contracts, Uvicorn/container execution, and Compose/reverse-proxy integration. A Lambda adapter is neither required nor part of the production path; see ADR 0012.
