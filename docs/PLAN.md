...
**Current phase; not yet complete.** Delivered as five independently reviewed increments: `narrative.events` and branch-aware effective history (revision 057–060), the `interaction.*` domain (061–062), real knowledge-provenance references closing Phase 5's placeholders (063), conditional-route evaluation (064), and `src/dnd_ai/commands` — the first application-layer command handlers (`RecordEvent`, `PerformInteraction`, `ResolveCheck`). A second, more critical exit review of that work then found production defects the first review's own verification loop missed: recorded events were not actually immutable (docs/ENTITY_LIFECYCLE.md §22's acceptance test was never written), `resolve_check()` had no concurrency control, interaction resolution was left incomplete (status/`resulting_event_id`/consequence never recorded), state provenance (`last_event_id`) had no timeline-safety check and was missing on three character-state tables `DATABASE_MODEL.md` §17 had already named, and the ruleset allow-list reverse guard and `test_role_grants.py`'s table coverage both had known gaps. A correction pass (revisions 065–068) closed all six and was opened as PR #16. A further review of that PR found three more production lifecycle gaps that survived both prior reviews: a recorded event could still be deleted outright (directly, or by deleting its `core.entities` row and letting `ON DELETE CASCADE` take the event with it); a completed interaction's `status` could revert from a terminal value back to `initiated`, silently reopening revision 067's append-only structural records to `UPDATE`/`DELETE`; and `resolve_check()` neither locked nor validated the parent interaction before recording a check result, so a check could be resolved against an interaction that had already finished, and an interaction with more than one `check_requests` row could be marked resolved after only the first one answered. A second correction pass (revisions 069–070) closed all three: `narrative.events` gained a `BEFORE DELETE` guard (blocking both the direct and the cascaded path); `interaction.interactions.status` became irreversible once terminal; and `resolve_check()` now locks the parent interaction, rejects resolving a check against an already-terminal one, and only transitions the interaction to `resolved` once every one of its check_requests has a result — that pass reached a confirmed green CI run. A further review then found the interaction stayed `initiated`, not `resolving`, between its first and last check result — the one status revision 067's structural-record guard treats as unlocked — so a check resolved first could already have produced an immutable event and state change while its own records stayed editable until every other check on the interaction answered too. A third correction pass (revision 071) closed it: `resolve_check()` now moves the interaction to `resolving` as soon as its first result lands, and a narrower database guard (`interaction.enforce_interaction_accepting_check_results()`) lets later check_results still be inserted during that status without weakening the existing `UPDATE`/`DELETE` protection. A fourth review then found two more gaps in the database itself: nothing advanced the status independently of `resolve_check()`, so a check result inserted through any other path could leave an interaction stuck at `initiated` with its structural records still unlocked; and `actions`/`targets`/`check_requests` could still be *created* after resolution began, letting a resolved interaction acquire a check it could never answer. A fourth correction pass (revision 072) closed both: `interaction.advance_interaction_status_on_check_result()`, an `AFTER INSERT` trigger on `check_results`, now owns the status transition atomically with the insert regardless of caller (making `resolve_check()`'s own status update redundant, so it was removed), and three new `BEFORE INSERT` triggers reuse revision 067's `enforce_interaction_locked()` to reject creating a new action, target, or check_request once the interaction has left `initiated`. Verified against AWS `dev`. `alembic check` clean throughout, one Alembic head. This working branch has not yet been merged, and its final-head CI run has not yet been confirmed — **do not mark this phase complete until [PHASE6_VERIFICATION.md](PHASE6_VERIFICATION.md)'s "Verification status" section says so.** Encounters/combat (§17's table shape) belongs to Phase 9, not this phase — see that phase's deliverable list. Verification evidence: [PHASE6_VERIFICATION.md](PHASE6_VERIFICATION.md).
