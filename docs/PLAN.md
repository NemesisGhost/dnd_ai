...
**Progress.** Workstream 1 (`080_security_identity_and_access`) delivered the `security.*` schema. Workstream 2 (`src/dnd_ai/domain/access.py`) delivered the effective-access resolver, including the rule that a campaign resolves access only against its own pinned timeline — never an unrelated timeline, branch, or a different world's — documented in that module and in [DATABASE_MODEL.md §19.7](architecture/DATABASE_MODEL.md#197-effective-access-resolution); a mismatch raises `UnauthorizedTimelineError`, mapped at the API boundary to a fixed, non-disclosing 404 rather than echoing the timeline/campaign IDs involved (`src/dnd_ai/domain/errors.py`'s `SafeMessageError`/`DomainAuthorizationError` classification — the same mechanism any future domain error uses to control its own client-facing message, rather than the API layer ever echoing a raw exception's text). Workstream 3 (`src/dnd_ai/api/`) delivered the FastAPI application skeleton: `/healthz` (process liveness) and `/readyz` (a real database round trip through the normal engine wiring, failing closed with a fixed, non-secret body and logging only the failure's exception class — never a DSN, host, or credential — safe for a Compose health check), a non-disclosing error contract (request-validation failures echo only a field's location and pydantic's error-type code; an unclassified domain error gets a fixed message, never `str(exc)`), correlation IDs, and one-transaction-per-request dependency. Application configuration (`src/dnd_ai/config.py`) is namespaced under `DND_AI_*` with an explicit allowlist for the test/CI/seed variables that share the namespace but belong to other subsystems (`DND_AI_TEST_DATABASE_URL`, `DND_AI_CI_DB_NAME`, `DND_AI_SEEDS_DIR`), and is genuinely fail-closed in production: `DND_AI_ENVIRONMENT=production` skips loading `.env` entirely and requires `DND_AI_DATABASE_URL` (a real environment variable or a mounted secret file named `dnd_ai_database_url`) — the legacy unprefixed `DATABASE_URL` alias and the local-dev default both remain local/test-only. Still to come are authentication verification, command/query endpoints and contracts, Uvicorn/container execution, and Compose/reverse-proxy integration. A Lambda adapter is neither required nor part of the production path; see ADR 0012.
