...
| 13 | Partially implemented | 13A (UI foundation) merged. 13B backend delivered: local accounts/activation/password reset, Argon2id passwords, opaque PostgreSQL browser sessions, CSRF/Origin enforcement, rate limiting, the authoritative `/auth/session` bootstrap contract (user, campaigns/roles/perspectives/capabilities, feature manifest), administrative account disable/reactivate/revoke-all-sessions, and durable local-auth/session security auditing; external OIDC bearer compatibility made optional in every environment, including production | Owner-authored portal integration against the bootstrap contract (13C onward), Foundry device-management/pairing UI, and Phase 12 feature gates remain; Phase 12-dependent surfaces stay disabled until that phase closes |
...
**Status: Partially implemented.** Implemented: the API, optional bearer-token verification, authorization, audience-filtered query contracts, the partially implemented Phase 12 services, and — as of the Phase 13B backend-readiness correction — local application authentication, the secure browser-session boundary (§23.4), the authoritative session-bootstrap contract, administrative account lifecycle (disable/reactivate/revoke-all-sessions), and durable local-auth/session security auditing. The hybrid Foundry pairing model's own migration/adapter changes are decided here and delivered under Phase 11R (§23.5); its device-management **UI** remains. 13A's owner-authored React portal shell is merged (fixture data only); the portal has not yet been wired to the bootstrap contract or any other live endpoint — that is 13C onward, still owner-authored, not started. A 13D backend-readiness audit (`docs/PHASE13D_BACKEND_READINESS.md`) found the Home/Characters/Quests screens' backend contracts already sufficient and added the two concrete gaps blocking the rest: session read endpoints (`GET /campaigns/{campaign_id}/sessions` list/`{session_id}` detail — previously only the GM-only `end_session` write existed) and a campaign quest list (`GET /campaigns/{campaign_id}/quests`, the read-only `get_quest_view` detail route already existed but nothing could enumerate a campaign's tracked quests); World-explorer and Knowledge-screen browsing remain without a backend (no list/search capability exists yet for locations, organizations, items, or knowledge views), tracked as a follow-up in that document. A follow-up World Explorer workstream (`docs/PHASE13D_WORLD_LOCATION_BROWSE.md`) delivered the first bounded slice of that follow-up: an authorized, type-filtered, searchable, keyset-paginated `GET /campaigns/{campaign_id}/locations` list over locations and dungeon areas, visible to a plain `campaign.view` caller only once published (`canon` canon status, `active` lifecycle status, not archived) and not specifically denied by a `campaign.view` resource grant, with a `canon.edit` holder additionally seeing their own non-canon (draft/proposed/etc.) definitions but never an archived/deleted/pending/inactive one — reusing the existing `dungeon_area` detail route throughout. Two pre-merge reviews corrected this workstream before it reached main: the first found and removed an unsound draft rule that inferred location-level "hidden" status from unrelated `knowledge.knowledge_items` rows; the second found that the endpoint as first proposed ignored `canon_status_id`/`lifecycle_status_id`/`archived_at` entirely, letting a plain player enumerate unpublished drafts (undetected because the test fixtures' own locations defaulted to draft status) — see that document's own corrected §3 and `dnd_ai.queries.location`'s docstring for both. Organizations, characters/NPCs, items, events, relationships, Knowledge browsing, a general (non-`dungeon_area`) location detail route, a separate archival/history-browsing surface, and any real location-level *discoverability* mechanism (an explicit schema/design decision, not yet made, and distinct from the publication-status gating now in place) remain outstanding, tracked in that document.
