...
| 13 | Partially implemented | 13A (UI foundation) merged. 13B backend delivered: local accounts/activation/password reset, Argon2id passwords, opaque PostgreSQL browser sessions, CSRF/Origin enforcement, rate limiting, the authoritative `/auth/session` bootstrap contract (user, campaigns/roles/perspectives/capabilities, feature manifest), administrative account disable/reactivate/revoke-all-sessions, and durable local-auth/session security auditing; external OIDC bearer compatibility made optional in every environment, including production. 13C delivered and verified: the portal replaced fixture identity data with the live bootstrap contract, adding campaign selection and explicit character perspective with capabilities sourced only from the bootstrap. 13D delivered and verified: live campaign-scoped Home, World, Characters, Quests (list/detail), Sessions (list/detail), and Knowledge screens with consistent loading/empty/denied/error/refreshing states | 13E (GM access tools), 13F (Foundry device-management/pairing UI), 13G (Phase 12 feature gates), and 13H (E2E coverage and packaging) remain; Phase 12-dependent surfaces stay disabled until that phase closes |
...
**Workstream 11R (code-complete, pending live verification):** delivered across nine bounded workstreams (A local accounts/passwords; B browser-session security; C unified `AuthenticatedPrincipal`/`FOUNDRY_ACCESS_AUTH_METHOD`; D pairing schema/commands, migration `100_foundry_pairing`; E management/pairing API endpoints; F bounded-adapter-surface conversion to `allow_foundry_access`; G audit attribution, migration `101_change_log_foundry_pairing`; H `foundry-module/` converted to per-device pairing at FoundryVTT minimum `13`; I `scripts/foundry_provision.py`'s legacy `FoundrySystem`-issuing subcommands retired in favor of a `pairing-code` subcommand over the same public pairing API), then hardened by a High-severity findings review: (1) Foundry scopes were persisted at pairing time but never enforced — closed by `AuthenticatedPrincipal.foundry_scopes`, `resolve_foundry_access_principal` re-reading `granted_scopes` fresh on every request, and `require_campaign_capability`'s new `foundry_scope` parameter, required (startup-time `ValueError` otherwise) on every `allow_foundry_access=True` route, with an explicit route-to-scope completeness test; (2) the legacy `FoundrySystem` scheme remained permanently, unconditionally accepted with no compatibility deadline and its own HTTP issuance endpoint still reachable — closed by unconditional rejection at `get_authenticated_user_id`, removal of `allow_foundry_system`/`issue_foundry_system_key_endpoint`, and migration `102_revoke_foundry_system_keys` revoking every existing legacy key at the data level (no compatibility window built — this repository found no evidence of a real deployed client still depending on the scheme). Item-by-item evidence, the bugs found and fixed while building it, and the full verification run (migrations `099`-`102`, quality gates, the full test suite, 77/77 FoundryVTT module tests) are recorded in `docs/PHASE11_VERIFICATION.md`. Live-Foundry closure testing must still exercise this path, not merely reconfirm the superseded `FoundrySystem` path — not yet performed, since it requires a licensed FoundryVTT v13 client no environment used for 11R's delivery had access to.
...
With that backend in place, the owner-authored **13C and 13D increments are complete and verified** on the `phase13d/main-ui` branch (`portal/README.md`'s "Phase 13C/13D verification"). 13C: `useSessionBootstrap`/`SessionProvider` replaced all fixture identity data with the live `/auth/session` bootstrap; `CampaignLayout`/`CampaignContextPanel` provide campaign selection and `CharacterPerspectiveContext` provides explicit per-campaign character perspective, both read from and validated against the bootstrap response; switching either navigates to the equivalent route under the new campaign/perspective and re-issues every campaign-scoped query, and capabilities (e.g. `access.manage` gating the Access nav item) come only from the bootstrap's per-campaign `capabilities` list — nothing is granted client-side. 13D: Home, World, Characters, Quests (list and detail), Sessions (list and detail), and Knowledge (view- and party-filtered, searchable, keyset-paginated) all read live campaign-scoped API data through shared `*Boundary` components that provide consistent loading, empty, denied/non-discoverable (`"unavailable"`), error-with-retry, and background-refreshing (`UpdatingIndicator`) states, with `AuthenticatedSessionBoundary` redirecting an expired or missing session to `/login` above all of them. Verification covers both the automated suite (369 portal tests via `npx vitest run` under `portal/`, plus `npm run lint`/`npm run build`, exercising denied/unavailable responses) and a live browser pass across GM, player, and observer/assistant-GM perspectives confirming correctly differing views, non-discoverable denied resources, list-to-detail navigation, and live search/filter/pagination. The Home dashboard is narrower than Phase 13's full Deliver bullet — it shows the latest session, previous-session recap, and recent events, but not yet active quests, recent discoveries, relevant NPCs/factions, reminders, or an Ask entry point (Ask itself remains a disabled placeholder pending Phase 12, per 13G); that gap is deferred rather than blocking 13D's closure. This still does **not** close Phase 13 as a whole — 13E (GM access tools), 13F (Foundry connections/devices UI), 13G (Phase 12 surfaces), and 13H (E2E coverage and packaging) have not started.
