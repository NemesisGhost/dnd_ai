...
Still to come: the deterministic audience-filtered summary/detail reads (GM/player-character/observer campaign summaries, docs/PLAN.md §25 step 15), the invitation-token acceptance flow and campaign-creation bootstrap deferred at workstream 20, the four resource-grant target kinds (`entity_id`/`knowledge_item_id`/`quest_id`/`session_id`/`event_id`) and character-relationship temporal bounds deferred above, and the actual reverse-proxy container (deliberately deferred to Phase 14 — see workstream 11 above). `apply_foundry_combat_sync`'s own HTTP exposure is also deliberately deferred, to Phase 11 (Foundry MVP), where a real Foundry-adapter caller and its authorization/transaction shape are designed together rather than retrofitted here — see workstream 10 above and `dnd_ai.commands.integration`'s own module docstring. A Lambda adapter is neither required nor part of the production path; see ADR 0012.
