...
| 11 | Partially implemented | Foundry-user identity linking (`link_foundry_identity`), adapter authentication (`issue_foundry_system_key`, `FoundrySystem` credential scheme — every existing command/query endpoint is Foundry-reachable), and a campaign-scoped combat-sync endpoint (`apply_foundry_combat_sync_endpoint`) | State-retrieval/restore endpoints, non-combat HP/condition/resource sync |
