...
Phase 4 is complete; [PHASE4_REMAINING_ISSUES.md](PHASE4_REMAINING_ISSUES.md) is a closed historical record. Phase 5 (Locations and dungeon play) delivered its first-time obligations (`character.characters.origin_location_id` and `campaign.character_location_history`, both recorded in [PLAN.md](PLAN.md#phase-5-locations-and-dungeon-play)); its production implementation is complete, while [PHASE5_REMAINING_ISSUES.md](PHASE5_REMAINING_ISSUES.md) remains open for a test-infrastructure teardown defect. The Phase 6 repository-context modularization gate is closed, but the Phase 5 formal-correctness gate remains open, so Phase 6 feature/schema work is still blocked.
