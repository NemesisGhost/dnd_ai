...
Phase 4 is complete; [PHASE4_REMAINING_ISSUES.md](PHASE4_REMAINING_ISSUES.md) is a closed historical record. Phase 5 (Locations and dungeon play) delivered its first-time obligations (`character.characters.origin_location_id` and `campaign.character_location_history`, both recorded in [PLAN.md](PLAN.md#phase-5-locations-and-dungeon-play)); it is complete, and [PHASE5_REMAINING_ISSUES.md](PHASE5_REMAINING_ISSUES.md) is now a closed historical record too. Both Phase 6 entry gates are closed, and Phase 6 feature/schema work is unblocked.
