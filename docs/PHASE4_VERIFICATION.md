...
Phase 4 is complete; [PHASE4_REMAINING_ISSUES.md](PHASE4_REMAINING_ISSUES.md) is a closed historical record. Phase 5 (Locations and dungeon play) delivered its first-time obligations (`character.characters.origin_location_id` and `campaign.character_location_history`, both recorded in [PLAN.md](PLAN.md#phase-5-locations-and-dungeon-play)); its production implementation is complete, but [PHASE5_REMAINING_ISSUES.md](PHASE5_REMAINING_ISSUES.md) remains open for one formal test-cleanup obligation. Phase 6 feature/schema work remains blocked until that obligation is fixed and verified.
