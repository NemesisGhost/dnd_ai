...
Phase 4 is complete; [PHASE4_REMAINING_ISSUES.md](PHASE4_REMAINING_ISSUES.md) is a closed historical record. Phase 5 (Locations and dungeon play) delivered its first-time obligations (`character.characters.origin_location_id` and `campaign.character_location_history`, both recorded in [PLAN.md](PLAN.md#phase-5-locations-and-dungeon-play)) and three corrections passes, but a fourth review reopened one concurrency issue — see [PHASE5_VERIFICATION.md](PHASE5_VERIFICATION.md) and [PHASE5_REMAINING_ISSUES.md](PHASE5_REMAINING_ISSUES.md). Phase 6 feature/schema work remains queued until that issue and its context-modularization entry gate close.
