...
Phase 4 is complete; [PHASE4_REMAINING_ISSUES.md](PHASE4_REMAINING_ISSUES.md) is now a closed historical record. Phase 5 (Locations and dungeon play) closed its first-time obligations (`character.characters.origin_location_id` and `campaign.character_location_history`, both recorded in [PLAN.md](PLAN.md#phase-5-locations-and-dungeon-play)) and all three of its exit-review corrections passes — two pre-merge, one post-merge — see [PHASE5_VERIFICATION.md](PHASE5_VERIFICATION.md); [PHASE5_REMAINING_ISSUES.md](PHASE5_REMAINING_ISSUES.md) is now a closed historical record too. [Phase 6](PLAN.md#phase-6-events-and-interactions) is current.
