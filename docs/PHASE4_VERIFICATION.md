...
Phase 4 is complete; [PHASE4_REMAINING_ISSUES.md](PHASE4_REMAINING_ISSUES.md) is a closed historical record. Phase 5 (Locations and dungeon play) delivered its first-time obligations (`character.characters.origin_location_id` and `campaign.character_location_history`, both recorded in [PLAN.md](PLAN.md#phase-5-locations-and-dungeon-play)); its production implementation and five-invariant concurrency suite are complete, but its formal test-infrastructure/tooling closeout is **reopened** pending a twelfth review's fixes passing CI on their final pushed head, and [PHASE5_REMAINING_ISSUES.md](PHASE5_REMAINING_ISSUES.md) is reopened to match. The Phase 6 repository-context modularization gate is closed; the Phase 5 formal-correctness gate is blocked pending that CI confirmation, so Phase 6 feature/schema work should not begin yet.
