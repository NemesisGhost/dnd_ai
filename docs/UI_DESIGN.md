...
- **`CampaignContextPanel`** answers "what campaign context and viewing perspective am I using?" It presents World, Campaign, Timeline, Time, and Viewing-as dimensions in that order, showing only dimensions backed by real authorized data (Campaign and Timeline come from the session bootstrap; World and Time stay absent until a real authorized value is supplied), and reuses the established character-perspective context/selector rather than introducing a second perspective state. It also carries a "Change campaign" link to the campaign chooser. It collapses behind a native disclosure control on narrow screens and is meant for a right-hand column or the shared shell.
