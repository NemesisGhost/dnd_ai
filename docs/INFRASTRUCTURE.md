...
9. ~~`scripts/aws-db-allow-my-ip.sh` and the CI IP-allowlist step don't exist yet~~ — resolved. Both exist (`scripts/aws-db-allow-my-ip.sh`; the "Open dev access for this runner"/cleanup steps in `.github/workflows/ci.yml`) and have been exercised manually against the deployed `dev` security group. Not yet confirmed: whether this repository's GitHub Actions secrets/variables (`AWS_CI_ROLE_ARN`, `DEV_DB_ADMIN_URL`, `AWS_REGION`, `DEV_DB_SECURITY_GROUP_ID`) are actually set — that determines whether the `aws-verification` CI job itself goes green, not whether the mechanism works.
