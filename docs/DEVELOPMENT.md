...
- Requires an explicit `--project`, `--env-file`, and at least one `--compose-file` on every invocation — never a default `.env`, never Compose's own project-name discovery. `--env-file`/every `--compose-file` must exist as a regular file, `--project` must be a syntactically valid Compose project name, and `--db-user`/`--db-name` must be non-empty — all checked before any subprocess runs.
- Builds every subprocess call from an explicit argument list (never a shell string), so a database name or file path can never be reinterpreted as a shell operator, and logs each one as an unambiguous `+ argv: [...]` list rather than a copy-paste shell command that may not even be correct for the active platform.
- Refuses to run if any of `--project`/`--env-file`/`--compose-file`/`--db-user`/`--db-name` still contains an unfilled placeholder like `<...>`, and refuses to target `postgres`/`template0`/`template1` as a restore or bootstrap-roles target.
- **`restore` and `bootstrap-roles` are split into a strict preflight phase and a mutation phase.** Preflight — argument/file validation, both `--confirm-drop`/`--confirm-restore` flags (checked before a single subprocess runs, so a missing flag touches nothing at all), Compose configuration rendering, server reachability, dump-archive validation (`pg_restore --list`, run against a uniquely named temporary copy inside the container, always cleaned up or, if cleanup itself fails, left in place with the run stopped), and an *active* check of what database a migration run would actually target — is entirely read-only and ends with a printed `PREFLIGHT PASSED — beginning destructive recovery` boundary. Nothing is dropped, created, or migrated before that boundary.
- **The migration target is verified by connecting, not by trusting a flag.** `bootstrap-roles`' `--confirm-env-targets-temp-db` and similar acknowledgment flags record operator intent; the script separately runs a one-off `python -c` check inside the `migrate` service's own container/environment (`SELECT current_database(), current_user`) and compares the result to the expected database/user before running any real migration. Neither the check nor its output ever include `DATABASE_URL`, `MIGRATION_DATABASE_URL`, or a password — only the resulting (non-secret) database and user names.
- `verify --check` executes each operator-supplied query with `PGOPTIONS=-c default_transaction_read_only=on` set for that one psql process — PostgreSQL itself, not a lexical `SELECT`-prefix scan, rejects any side-effecting statement (confirmed: `SELECT lo_create(-1)` fails with `cannot execute lo_create() in a read-only transaction`). The lexical check still requires exactly one non-empty `SELECT` statement and rejects multi-statement input, as a first-line guard against obvious mistakes — it is not what makes the execution read-only.
- Prints the selected project, environment file, Compose files, and database before doing anything destructive, and never prints `docker compose config`'s raw output — only non-secret structural fields (service names, port lists) extracted from its parsed JSON.
...
- Never deletes a volume, and never deletes evidence (an in-container dump copy, a temporary bootstrap database) left behind by a failed operation — inspect it, then clean it up by hand once you're done. When a non-essential cleanup step fails *after* the primary operation already succeeded (removing an in-container temp copy after a successful backup or restore), the command still exits 0 but prints an explicit `... succeeded WITH CLEANUP WARNING` message naming the leftover path — it never prints an unconditional "complete" message that would hide that. `bootstrap-roles` is the one case where a stuck temporary database after a *verified-successful* bootstrap is itself something requiring operator action, so that specific case exits nonzero with a message distinguishing "bootstrap succeeded" from "cleanup failed, temporary database remains."
- `preflight` and `validate-archive` expose those same read-only checks as their own standalone commands — run `preflight --for restore-fresh|restore-existing|bootstrap-roles` or `validate-archive` independently of any mutation, any time.
...
In fresh-cluster mode, `restore` runs a strict, read-only **preflight** phase before touching anything: both `--confirm-drop`/`--confirm-restore` are checked first (before any subprocess at all — a missing flag means literally nothing has run yet), then Compose configuration, server reachability, `--dump-file` archive validation (`pg_restore --list` against a temporary in-container copy), and an *active* check — a live connection through the `migrate` service's own configuration, not a guess — confirming `MIGRATION_DATABASE_URL` really targets this database. Only once all of that passes does it print `PREFLIGHT PASSED — beginning destructive recovery` and start **mutating**:

1. Runs the `migrate` service directly against the database that PostgreSQL's own container init already created — this **is** the "temporary/bootstrap application database": on a genuinely fresh cluster it holds nothing yet, so migrating it to create the six roles, extensions, schemas, and current schema objects costs nothing. (This is the first mutating step, and it only runs after every preflight check above — including both confirmation flags — has already passed.)
...
Run `preflight --for restore-fresh --dump-file <dump>` (plus the same `--project`/`--env-file`/`--compose-file`/`--db-user`/`--db-name`) any time to see exactly these checks — and nothing else — without restoring anything.
...
This creates `dnd_ai_bootstrap_tmp`, **actively verifies** — by connecting through the `migrate` service's own configuration and comparing `current_database()`/`current_user()` — that `MIGRATION_DATABASE_URL` really does target it before running anything, then migrates it (creating the six roles), verifies them, and — only once verification passes — drops that temporary database again. `--protect-db-name` is a safety guard: `bootstrap-roles` refuses to run if `--temp-db-name` equals it. `--confirm-env-targets-temp-db` is a required acknowledgment that `.env.bootstrap`'s `MIGRATION_DATABASE_URL` is *intended* to point at the temporary database — the script still never opens that file to check for you, but it no longer merely trusts the flag either: if the active check finds `MIGRATION_DATABASE_URL` actually points somewhere else (the real recovery target, for instance), migrations are never run, the temporary database is dropped again, and the command exits nonzero explaining the mismatch. If role verification fails after migrating (a different, later failure), the temporary database is left in place for inspection rather than silently dropped, and the command distinguishes "bootstrap succeeded, cleanup failed" from an outright failure in its final message.
