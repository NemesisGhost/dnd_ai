...
`verify` has no `--confirm-*` flag and never runs migrations, unconditionally — its acceptance battery includes `alembic current --check-heads` (requires `alembic>=1.17.1`, this project's declared minimum), which proves the database's recorded revision(s) genuinely equal the migration scripts' current head(s), not merely that `core.alembic_version` is readable.
