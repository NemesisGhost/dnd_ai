...
`verify` has no `--confirm-*` flag and never runs migrations, unconditionally — its acceptance battery includes a read-only migration-head check that proves the database's recorded revision(s) genuinely equal the migration scripts' current head(s), not merely that `core.alembic_version` is readable. It deliberately does not shell out to `alembic current --check-heads`: that command loads `database/migrations/env.py`, whose `run_migrations_online()` unconditionally creates and commits the `core` schema — real mutation Alembic's own `dont_mutate=True` does not suppress. Instead it reads repository heads via Alembic's `ScriptDirectory` (no env.py) and database heads via a read-only-enforced query against `core.alembic_version` directly (see the dedicated guide).
