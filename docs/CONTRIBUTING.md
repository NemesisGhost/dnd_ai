...
Phases 1 through 4 are complete and CI-verified. Phase 5's production implementation is complete, but its formal verification remains open in [PHASE5_REMAINING_ISSUES.md](PHASE5_REMAINING_ISSUES.md) pending a ninth review's redesign of the test-worker cleanup helper's startup and cancellation containment. The [Phase 6 entry gates](PLAN.md#phase-6-events-and-interactions) are therefore split: repository-context modularization is closed, while Phase 5 formal correctness remains open. Every phase verifies against the deployed `dev` RDS instance (migrations, `tests/database`, `tests/scenario`) — see [PLAN.md §23.0](PLAN.md#230-aws-verification-policy) and [§29.9](PLAN.md#299-aws-first-verification-mechanism) for why and how. A local PostgreSQL container is a documented fallback for when AWS is genuinely unreachable, not the default path — see [DEVELOPMENT.md §3](DEVELOPMENT.md#3-local-setup).
