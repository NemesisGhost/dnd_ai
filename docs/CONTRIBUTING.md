...
Phases 1 through 4 are complete and CI-verified, including all three of Phase 4's closeout passes ([PHASE4_REMAINING_ISSUES.md](PHASE4_REMAINING_ISSUES.md) is a closed historical record). Phase 5's implementation and three corrections passes are merged and CI-verified, but [PHASE5_REMAINING_ISSUES.md](PHASE5_REMAINING_ISSUES.md) is reopened for one remaining concurrency race. Close it and the [Phase 6 context-modularization gate](PLAN.md#phase-6-events-and-interactions) before beginning Phase 6 feature/schema work. Every phase verifies against the deployed `dev` RDS instance (migrations, `tests/database`, `tests/scenario`) — see [PLAN.md §23.0](PLAN.md#230-aws-verification-policy) and [§29.9](PLAN.md#299-aws-first-verification-mechanism) for why and how. A local PostgreSQL container is a documented fallback for when AWS is genuinely unreachable, not the default path — see [DEVELOPMENT.md §3](DEVELOPMENT.md#3-local-setup).
