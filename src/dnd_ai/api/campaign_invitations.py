...
caller authenticates with `dnd_ai.api.auth.get_authenticated_user_id`
only, and the invitation *token itself* (in the request body, never the
URL, so it never lands in access logs) is the authorization credential,
the same "no campaign yet" shape `dnd_ai.api.campaigns.create_campaign_
...
def accept_campaign_invitation_endpoint(
    ...
    accepting_user_id: Annotated[uuid.UUID, Depends(get_authenticated_user_id)],
    connection: Annotated[Connection, Depends(get_connection)],
    correlation_id: Annotated[str | None, Depends(get_request_correlation_id)],
